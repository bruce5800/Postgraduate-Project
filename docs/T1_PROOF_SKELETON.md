...
| **Lemma 3** (tolerant-testing lower bound) | **W2 CONFIRMED** (`docs/T1_W2_W3a_closeout.md`): Canonne–Jain–Kamath–Li 2022 — tolerant identity testing is Θ̃(n/log n) at constant tolerance (vs √n non-tolerant); our construction is provably tolerant (a>0). |
...
| **W3** (heterogeneous construction, Lemma 4) | **Closed to one routine step** (`docs/T1_W3_construction.md` + `T1_W2_W3a_closeout.md`): reduced to tolerant-identity testing (W2 confirmed). **W3a is now an EXACT AFFINE LAW** `follow-ratio = ρ_perfect − ½·L1(p,q)` (proven + verified), which also guarantees the good side is a Θ(1) ball (a>0, the tolerant regime). Found the low-dim version too weak (dimension essential). Remaining: name the two VV witness distributions at L1=a, b (routine — the affine law makes any such pair work). |
