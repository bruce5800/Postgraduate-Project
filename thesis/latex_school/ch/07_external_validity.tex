...
count, \(0.108\) ms, about \(2\%\) of the cost of computing
\(\mathrm{OPT}\) once.
...
(Caltech36) to \(0.11\) (CE-PG). \textbf{F3 holds on all six, and
confirms its own logic}: the consistency upside is small everywhere
(mean \(+0.049\); range \(+0.022\)--\(+0.077\)) and smallest exactly
where the baseline is strongest --- the two dense economic graphs, with
...
below it. Those two graphs are so dense that matching is nearly trivial
(Ranking \(\approx0.97\), MinDegree \(=1.00\)), so there is neither
upside to capture (F3) nor much downside to protect: the boundary is
F3's own mechanism at work, not an exception to it. Finally, F4 is
