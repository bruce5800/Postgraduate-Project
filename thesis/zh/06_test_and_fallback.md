...
将 $\tau$ 重校准到**测得**的基线 $\hat\beta$ 可消除该病理：在边界建议处，最坏情况阈值的误判随前缀增长
从 $0.03$ 攀到 $1.00$、比值降到 $0.920$，而重校准阈值在每个前缀规模上都保持误判 $0.00$、比值 $\approx0.986$。
...
`tests/test_combiner_small.py`）**二者**——因为中途从 Ranking 切到 Mimic 会使已提交的匹配落入一个
