...
narrower (correlations \(0.67\) and \(0.85\)); where they are not ---
blind following against Ranking under garbage advice, correlation
\(-0.15\) --- pairing does not help and the per-algorithm interval is
the honest one. The narrowest margin in the thesis, Feldman(MPD)'s
\(+0.0044\) against a \(\pm0.0023\) per-algorithm interval, tightens to
...
(max diff 0.0017 ER, 0.0013 LR --- the paper omits Ranking's curve for
...
\item
  ...
  --- it falls off linearly in the advice's \(\ell_1\) error --- again
  to three decimals. In the language of §10.2 this is what makes the
