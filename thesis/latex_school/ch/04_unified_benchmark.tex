...
\tightlist
\item
  \textbf{Panel A --- clvb\_zipf} (\(n=1000\), Zipf exponent \(1.0\), 60
  trials): heavy-tailed offline degrees, where degree predictions carry
  signal.
\item
  \textbf{Panel B --- left-regular \(d{=}5\)} (\(n=1000\), 60 trials):
  near-homogeneous degrees --- the hard case of Chapter 3 --- where
  predictions have little signal to carry.
\item
  \textbf{Panel C --- few-types \(r{=}8\)} (\(n=2000\), 50 trials, test
  prefix \(k=200\)): near-perfect-matchable, the calibrated regime for
  the type-histogram test-and-fallback algorithms.
\end{itemize}

\textbf{Shared settings.} Every panel runs on the harness of Chapter 3
with \(m=n\) i.i.d. arrivals per instance and \emph{paired trials}:
