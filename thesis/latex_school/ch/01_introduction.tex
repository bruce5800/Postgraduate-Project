...
\begin{itemize}
\item
  \textbf{A unified experimental benchmark} (Chapter 4; Q1): the first
...
\item
  ...
  (Chapters 4, 7; Q1): unguarded following crashes \emph{below} the
...
\item
  ...
  Q2): its testing cost, a threshold-calibration pathology in which a
...
\item
  ...
  We trace it to a \emph{resolution limit} --- no practical threshold
  can capture an upside below its own estimator's noise --- which
  persists across the whole difficulty range (Figure 6.4).
\end{itemize}
...
(10). One sentence recurs throughout, and §10.1 states it in full:
\textbf{on average-case online matching, predictions are robustness
