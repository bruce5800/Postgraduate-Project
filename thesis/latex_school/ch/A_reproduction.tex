...
Greedy \(0.916\) against \(0.917\) on \path|clvb_zipf| and \(0.889\)
against \(0.890\) on \path|left_regular|; MPD under a perfect
...
machine-generated into \path|results/streaming_ladder_tables.md|;
...
\path|left_regular|. Both produce maximal matchings, so both are
...
\path|few_types|, where one pass reaches \(0.978\) against Greedy's
...
\bottomrule\noalign{}
\endlastfoot
\path|clvb_zipf| & \(+0.074\) & \(+0.069\) & \(+0.072\) \\
\path|left_regular| & \(+0.042\) & \(+0.079\) & \(+0.087\) \\
\path|few_types| & \(+0.047\) & \(+0.013\) & \(+0.017\) \\
...
exceptions are \path|clvb_zipf|, where the two are within \(0.005\),
and \path|few_types|, the family engineered to be near-perfectly
