...
histogram under \emph{perfect} advice, i.e.~pure sampling noise, which
falls as \(k\) grows and spans \(\approx0.13\) down to \(\approx0.05\)
over the prefix lengths swept here, so it can never confidently
\emph{accept}: it rejects everything, including perfect advice, and
always plays the baseline. In short:
...
\emph{safely capture} stays near zero, because the empirical test's
