...
![M0：用发散特征时，rank 训练在匹配比值上击败回归（左），尽管其回归拟合更差（右）——这一解离表明回归对 MPD 是错误的训练目标。](../../results/rank_vs_mse_mve.png){width=100%}
...
![M3——真实 trace（Azure LLM）上的决定性负结果：在真实到达与真实时序特征下，rank 训练与 MSE 训练的预测器彼此不可区分、也与未学习的上窗口计数不可区分，全都贴近预言机——驱动 rank 训练的人造顺序/幅度发散并不出现。](../../results/rank_real_trace.png){width=100%}
...
![服务 SLO 探针：在突发负载下的紧 SLO/尾部目标上，一个非预测策略（静态余量或反应式自适应）在几个百分点内匹配 clairvoyant 预言机——前瞻无济于事。](../../results/serving_slo_probe.png){width=75%}
