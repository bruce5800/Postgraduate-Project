...
script. Two classes must be distinguished. Most scripts are
\textbf{self-contained}: they generate their own synthetic instances and
run as-is on a clean checkout. The rest --- the real-graph and
real-trace results of Sections 6 and 8 --- first need external data
placed under \texttt{data/}, which we do not redistribute; §A.4 records
what each dataset is and where it comes from. A dagger (\(\dagger\))
marks that second class in the map below.
...
\textbf{Fig. 6} (real predictor) \(\dagger\) & \texttt{real\_predictor}
& 15 s \\
...
\texttt{rank\_when\_it\_matters}, \texttt{rank\_real\_trace} \(\dagger\)
& 40 s \\
...
§8 serving case study \(\dagger\) & \texttt{serving},
\texttt{serving\_trace}, \texttt{serving\_dynamic},
\texttt{prefix\_cache}, \texttt{serving\_slo\_probe} & varies \\
...
External data lives under \texttt{data/} and is not redistributed with
the code.
