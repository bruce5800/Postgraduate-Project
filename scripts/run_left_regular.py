"""Reproduce Borodin et al. (2018) Random Left-Regular experiments (Fig 18).

Algorithms: SimpleGreedy, Ranking, FeldmanEtAl(/g), JailletLu(/g).
Type graphs: every ℓ has degree exactly d, with d ∈ {1, 2, ..., 30}.
n = 1000, 100 trials per d. Paper identifies d=5 as the "difficult case".
"""
...
from graphs.synthetic import left_regular_bipartite
...
def main(n: int = 1000, n_trials: int = 100, seed: int = 0) -> None:
    ...
    for di, d in enumerate(ds):
        ...
        for _ in range(n_trials):
            type_adj = left_regular_bipartite(n, d, rng_graph)
        ...
        eta = elapsed / (di + 1) * (len(ds) - di - 1)
        line.append(f"elapsed={elapsed:5.1f}s  eta={eta:5.1f}s")
        print("  ".join(line))

    out_json = out_dir / "left_regular.json"
    ...
    try:
        ...
        for a, (label, marker, ls, color) in style.items():
            ...
            ax.plot(ds, m, label=label, marker=marker, markersize=4, linestyle=ls, color=color)
            ax.fill_between(ds, m - s, m + s, alpha=0.12, color=color)
        ax.set_xlabel("d (left-degree)")
        ax.set_ylabel("competitive ratio (alg / OPT)")
        ax.set_title(f"Left-Regular bipartite, n={n}, {n_trials} trials per d — Borodin et al. (2018) Fig 18 partial reproduction")
        ax.grid(True, alpha=0.3)
        ax.legend(loc="lower right", ncol=2)
        out_png = out_dir / "left_regular.png"
